...
k
s
i % i is the increment of the exponent of A when the Ising model fails to find a valid solution
d  is square euclidean distance, 2 is manhattan distance
n
x1 y1
x2 y2
. .
...
### loosening the constraints
   ...
   2. To maintain the constraints of the traveling salesman problem, the formula $$\frac{A}{B} >= max(W_{uv}) - (1)$$ needs to be satisfied. *max(W<sub>uv</sub>)* in this problem is the maximum distance between two nodes, and let it be *d<sub>max</sub>*. We can simply set $$A = max(d_{max})$$ and $$B = 1$$ for simplicity.
   ...
   4. We first set $$A = d_{max}^init$$, where *init* is the initial value of the exponent and *init < 1*. We use the Ising model to find the route and check its validity. If it is valid, then it is the answer. Otherwise, we increases *A* by *d<sub>max</sub><sup>inc<sup>*. That is, $$A = A x d_{max}^{inc}$$
